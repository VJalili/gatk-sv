...
:::info
Note that a list of sample IDs must be provided.
Refer to the [sample ID requirements](../getting_started/requirements)
for specifications of allowable sample IDs.
IDs that do not meet these requirements may cause errors.
:::

## Inputs:
* Per-sample BAM or CRAM files aligned to hg38. Index files
(`.bai`) must be provided if using BAMs.
